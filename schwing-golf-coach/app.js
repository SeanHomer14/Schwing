...
const state = {
  ...
  profile: JSON.parse(localStorage.getItem("loopProfile") || "null") || { ...defaultProfile },
  memories: JSON.parse(localStorage.getItem("loopMemories") || "[]"),
  swingImageData: "",
  swingFrameData: [],
  player: JSON.parse(localStorage.getItem("schwingPlayer") || "null") || {
    code: "",
    displayName: "",
    cloudEnabled: false,
    status: "Local only"
...
const playerCodeInput = document.querySelector("#playerCode");
const playerDisplayNameInput = document.querySelector("#playerDisplayName");
const playerCloudTitle = document.querySelector("#playerCloudTitle");
const playerCloudStatus = document.querySelector("#playerCloudStatus");
const loginPlayerForm = document.querySelector("#loginPlayerForm");
const loginPlayerCodeInput = document.querySelector("#loginPlayerCode");
...
function savePlayerLocal() {
  localStorage.setItem("schwingPlayer", JSON.stringify(state.player));
}

function playerLabel() {
  if (!state.player.code) return "Local only";
  return state.player.displayName || state.player.code;
}

function setCloudStatus(message, isError = false) {
  state.player.status = message;
  playerCloudStatus.textContent = message;
  playerCloudStatus.classList.toggle("error-text", isError);
  loginStatus.textContent = message;
  loginStatus.classList.toggle("error-text", isError);
  savePlayerLocal();
}

function renderPlayerCloud() {
  const emptyProfileStatus = "Enter a player code to load or create a cloud profile.";
  const profileStatus = state.player.code ? state.player.status : emptyProfileStatus;
  const loginStatusText = state.player.code ? state.player.status : "Enter your player code to load your cloud profile.";
  playerCodeInput.value = state.player.code || "";
  playerDisplayNameInput.value = state.player.displayName || "";
  loginPlayerCodeInput.value = state.player.code || "";
  loginDisplayNameInput.value = state.player.displayName || "";
  playerCloudTitle.textContent = state.player.code ? playerLabel() : "Local profile";
  playerCloudStatus.textContent = profileStatus || emptyProfileStatus;
  playerCloudStatus.classList.remove("error-text");
  loginTitle.textContent = state.player.code ? `Welcome back, ${playerLabel()}` : "Who is playing?";
  loginStatus.textContent = loginStatusText || "Enter your player code to load your cloud profile.";
  loginStatus.classList.remove("error-text");
  loginContinueButton.hidden = !state.player.code;
  loginContinueButton.textContent = state.player.code ? `Continue as ${playerLabel()}` : "Continue as player";
  const homePlayerSummary = document.querySelector("#homePlayerSummary");
  if (homePlayerSummary) {
    homePlayerSummary.textContent = state.player.cloudEnabled ? playerLabel() : "Local only";
  }
}

function openApp(panelId = "home") {
...
async function loadCloudPlayer(code) {
  ...
  const data = await response.json().catch(() => ({
    mode: "error",
    answer: "Cloud profile returned an unreadable response."
  }));
  if (!response.ok) throw new Error(data.answer || "Cloud profile unavailable");
  return data;
}

async function saveCloudPlayer() {
  if (!state.player.cloudEnabled || !state.player.code || state.isLoadingCloud) return;
  ...
  if (!data.configured) {
    ...
    setCloudStatus("Cloud is not connected on Render yet. Local profile is still saved.", true);
    renderPlayerCloud();
    return;
  }
  if (data.player?.updatedAt) {
    setCloudStatus(`Saved to cloud ${new Date(data.player.updatedAt).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}.`);
  } else {
...
function scheduleCloudSave() {
  if (!state.player.cloudEnabled || state.isLoadingCloud || state.isBooting) return;
  window.clearTimeout(state.saveTimer);
  state.saveTimer = window.setTimeout(() => {
    saveCloudPlayer().catch((error) => setCloudStatus(error.message, true));
  }, 800);
}
...
async function loadPlayerProfile(codeInput, displayNameInput, options = {}) {
  const code = cleanPlayerCode(codeInput);
  const displayName = displayNameInput.trim();
  if (!code || code.length < 3) {
    setCloudStatus("Use at least 3 letters or numbers for the player code.", true);
    return;
  }
  ...
  try {
    const data = await loadCloudPlayer(code);
    if (!data.configured) {
      state.player = { code, displayName, cloudEnabled: false, status: "Cloud is not connected on Render yet. Local profile is still saved." };
      savePlayerLocal();
      renderPlayerCloud();
      return;
    }
    ...
    state.player = {
      code,
      displayName: data.player?.displayName || displayName || code,
      cloudEnabled: true,
      status: data.player ? "Cloud profile loaded." : "New cloud profile ready. Tap Create profile."
    };

    if (data.player) {
      state.profile = { ...defaultProfile, ...(data.player.profile || {}) };
      state.clubs = Array.isArray(data.player.clubs) && data.player.clubs.length ? data.player.clubs : defaultClubs.map((club) => [...club]);
      state.memories = Array.isArray(data.player.memories) ? data.player.memories : [];
      localStorage.setItem("loopProfile", JSON.stringify(state.profile));
      localStorage.setItem("loopClubs", JSON.stringify(state.clubs));
      localStorage.setItem("loopMemories", JSON.stringify(state.memories));
    }
    ...
    renderProfile();
    renderMemories();
    if (options.openAfterLoad && data.player) {
      openApp("home");
    }
  } catch (error) {
...
async function savePlayerProfile(codeInput, displayNameInput, options = {}) {
  const code = cleanPlayerCode(codeInput || state.player.code);
  const displayName = displayNameInput.trim();
  if (!code || code.length < 3) {
    setCloudStatus("Use at least 3 letters or numbers for the player code.", true);
    return;
  ...
  state.player.displayName = displayName || state.player.displayName || code;
  state.player.cloudEnabled = true;
  savePlayerLocal();
  renderPlayerCloud();
  setCloudStatus("Saving cloud profile...");
  try {
...
document.querySelector("#playerCodeForm").addEventListener("submit", async (event) => {
  event.preventDefault();
  await loadPlayerProfile(playerCodeInput.value, playerDisplayNameInput.value);
});

document.querySelector("#savePlayerButton").addEventListener("click", async () => {
  await savePlayerProfile(playerCodeInput.value, playerDisplayNameInput.value);
});

loginPlayerForm.addEventListener("submit", async (event) => {
  event.preventDefault();
  await loadPlayerProfile(loginPlayerCodeInput.value, loginDisplayNameInput.value, { openAfterLoad: true });
});

document.querySelector("#loginSaveButton").addEventListener("click", async () => {
  await savePlayerProfile(loginPlayerCodeInput.value, loginDisplayNameInput.value, { openAfterSave: true });
});
...
document.querySelector("#loginLocalButton").addEventListener("click", () => {
  state.player.cloudEnabled = false;
  state.player.status = "Using local profile on this phone.";
  savePlayerLocal();
  renderPlayerCloud();
  openApp("home");
});
