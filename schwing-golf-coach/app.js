...
function requireServerMode() {
  if (window.location.protocol === "file:") {
    throw new Error("Open Schwing from the Render link, not the local HTML file. Cloud profiles and AI need the app server.");
  }
}
...
async function loadPlayerProfile(codeInput, displayNameInput, options = {}) {
  ...
  try {
    ...
    if (!data.configured) {
      ...
      savePlayerLocal();
      renderPlayerCloud();
      setCloudStatus(state.player.status, true);
      return;
    }
