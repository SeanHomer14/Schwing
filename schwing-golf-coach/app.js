...
function renderPlayerCloud() {
  ...
  playerCloudStatus.textContent = state.player.status || "Enter a player code to load or create a cloud profile.";
...
document.querySelector("#playerCodeForm").addEventListener("submit", async (event) => {
  event.preventDefault();
  const code = cleanPlayerCode(playerCodeInput.value);
  const displayName = playerDisplayNameInput.value.trim();
  ...
  try {
    ...
    state.player = {
      ...
      cloudEnabled: true,
      status: data.player ? "Cloud profile loaded." : "New cloud profile ready. Tap Save to cloud."
    };
...
document.querySelector("#savePlayerButton").addEventListener("click", async () => {
  const code = cleanPlayerCode(playerCodeInput.value || state.player.code);
  const displayName = playerDisplayNameInput.value.trim();
  if (!code || code.length < 3) {
    setCloudStatus("Use at least 3 letters or numbers for the player code.", true);
    return;
