...
const supabaseTable = process.env.SUPABASE_PLAYERS_TABLE || "schwing_players";
...
function rowToPlayer(row) {
  ...
  return {
    ...
    displayName: row.display_name || row.player_code,
    profile: row.profile || {},
    clubs: row.clubs || [],
...
async function loadCloudPlayer(code) {
  const url = `${supabaseUrl}/rest/v1/${supabaseTable}?player_code=eq.${encodeURIComponent(code)}&select=*`;
  const response = await fetch(url, {
    method: "GET",
    headers: supabaseHeaders()
  });
  if (!response.ok) {
    throw new Error(`Supabase load error ${response.status}: ${await response.text()}`);
  }
  const rows = await response.json();
  return rowToPlayer(rows[0]);
}

async function saveCloudPlayer({ code, displayName = "", profile = {}, clubs = [], memories = [] }) {
  const url = `${supabaseUrl}/rest/v1/${supabaseTable}?on_conflict=player_code`;
  const response = await fetch(url, {
    method: "POST",
  ...
  if (!response.ok) {
    throw new Error(`Supabase save error ${response.status}: ${await response.text()}`);
  }
  const rows = await response.json();
  return rowToPlayer(rows[0]);
}
...
const server = http.createServer(async (req, res) => {
  if ((req.method === "GET" || req.method === "HEAD") && req.url === "/health") {
    sendJson(res, 200, {
      ...
      model,
      cloudProfiles: cloudConfigured()
    }, req.method !== "HEAD");
    return;
  }

  if (req.method === "POST" && req.url === "/api/player/load") {
    try {
      const payload = await readJson(req);
      const code = normalizePlayerCode(payload.code);
      if (!isValidPlayerCode(code)) {
        sendJson(res, 400, { mode: "error", answer: "Player code must be 3-24 letters or numbers." });
        return;
      }
      if (!cloudConfigured()) {
        sendJson(res, 200, { mode: "local", configured: false, player: null });
        return;
      }
      const player = await loadCloudPlayer(code);
      sendJson(res, 200, { mode: "cloud", configured: true, player });
    } catch (error) {
  ...
  if (req.method === "POST" && req.url === "/api/player/save") {
    try {
      const payload = await readJson(req);
      const code = normalizePlayerCode(payload.code);
      if (!isValidPlayerCode(code)) {
      ...
      const player = await saveCloudPlayer({
        code,
