...
function saveClubs() {
  ...
  activeClubCount.textContent = state.clubs.filter(([, distance]) => Number(distance) > 0).length;
}
...
function renderMemories() {
  ...
  if (!state.memories.length) {
    ...
    empty.textContent = "Save round notes here and Loop will use the most recent ones when answering.";
